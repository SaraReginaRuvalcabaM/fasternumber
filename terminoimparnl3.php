...
<body>
   ...
   ?>   <h1>¡Se ababo el tiempo!</h1>
       ...
       <?php if ($reg2['count(id_registro)']!=0){
         $instru_SQL ="UPDATE puntajejuegosimpares SET num_impares_pun_nl3='$contador' where id_registro='$usuariois';";
         $re3=mysqli_query($connexion,$instru_SQL); }
        else{
            $instru_SQL = "INSERT INTO puntajejuegosimpares(id_registro,num_impares_pun_nl3) VALUES('$usuariois','$contador');";
            $re4=mysqli_query($connexion,$instru_SQL);
        }
       
    ?>
    <input type=button value="Ver tabla de puntuaciones más altas" class=usuario onclick="top.juego.location='registropuntajeimparnl3.php'">
    <input type=button value="Volver a jugar" class=usuario onclick="top.juego.location='juegoimparnl3.html'">
